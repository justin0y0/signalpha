...
function Body({ data }: { data: Detail }) {
  ...
  const fmt = (v: any, digits = 2, prefix = '') => {
          ...
          <div className="tdp-head__meta">
            {p ? (
              <>
                ...
                {p.sector && <span className="tdp-head__sector" style={{ color: '#a78bfa' }}>{p.sector}</span>}
      ...
      {p && (
        <motion.div className="tdp-card"
          ...
          <div className="tdp-grid">
            ...
            <Stat label="β market" value={fmt(p.beta_market, 2)} />
            <Stat label="β sector" value={fmt(p.beta_sector, 2)} />
          ...
          <div className="tdp-factors">
            {sig.factors.map((f: any, i: number) => (
              <div key={i} className="tdp-factor">
                <span>{f?.label || '—'}</span>
                <b style={{ color: (f?.value || 0) >= 0 ? '#4ade80' : '#f87171', fontFamily: 'var(--font-mono)' }}>
                  {fmtSigned(f?.value, 2)}
                </b>
              </div>
            ))}
            ...
            {earnings.map((e: any, i: number) => (
              <div key={i} className="tdp-eh__row">
                ...
                <div className="tdp-eh__move mono" style={{
                  ...
                  {typeof e?.actual_move_pct === 'number'
